...
import { AutoGTranslator } from "./auto-translator-google";
import { AutoYTranslator } from "./auto-translator-yandex";
...
export class I18nGenerator implements IDisposable, InsertActionProviderDelegate {
    ...
    async generateYTranslateApiCodeAdd(): Promise<void> {
        const config = await this.readConfigFileAsync();
        const apiKey = await this.ua.promptAsync(
        ...
        config.yandexTranslateApiKey = apiKey;

        await this.initializeAsync(config);
        await this.writeConfigFileAsync(config);
        this.ua.showInfo(`Saved Yandex Translate API key.`);
    }

    async generateGTranslationsAsync(): Promise<void> {
        const config = await this.readConfigFileAsync();

        await this.initializeAsync(config);
        if (await this.generateAutoGTranslationsAsync(config)) {
            this.ua.showInfo(`Translations created.`);
            await this.generateUpdateAsync();
        }
    }

    async generateYTranslationsAsync(): Promise<void> {
        const config = await this.readConfigFileAsync();

        await this.initializeAsync(config);
        if (await this.generateAutoYTranslationsAsync(config)) {
            this.ua.showInfo(`Translations created.`);
            await this.generateUpdateAsync();
        }
    }
    ...
    private async generateAutoGTranslationsSetAsync(
        translator: AutoGTranslator,
        locale: string,
        ...
        for (let availableTranslation of availableKeys) {
            ...
            if (typeof value === "object") {
                ...
                if (!await this.generateAutoGTranslationsSetAsync(translator, locale, Object.keys(value), value, translatedValue)) {
                    return false;
                }
            } else {
                if (index === -1) {
    ...
    private async generateAutoYTranslationsSetAsync(
        translator: AutoYTranslator,
        locale: string,
        availableKeys: string[],
        defaultSet: any,
        translationSet: any): Promise<boolean> {
        ...
        for (let availableTranslation of availableKeys) {
            ...
            if (typeof value === "object") {
                ...
                if (!await this.generateAutoYTranslationsSetAsync(translator, locale, Object.keys(value), value, translatedValue)) {
                    return false;
                }
            } else {
                if (index === -1) {
    ...
    private async generateAutoGTranslationsAsync(config: I18nConfig): Promise<boolean> {
        // Get all the default translations
        const defaultI18n = await this.readI18nFileAsync(
            config.defaultLocale || "", config, true
        );
        // Initialize Translator
        const translator: AutoGTranslator = new AutoGTranslator(config);
        ...
        for (const locale of config.locales) {
            if (locale === config.defaultLocale) {
                ...
                try {
                    ...
                    const result = await this.generateAutoGTranslationsSetAsync(translator, locale, availableKeys, defaultI18n, translations);
                    if (!result) {
                        return result;
                    }
    ...
    private async generateAutoYTranslationsAsync(config: I18nConfig): Promise<boolean> {
        // Get all the default translations
        const defaultI18n = await this.readI18nFileAsync(
            config.defaultLocale || "", config, true
        );
        // Initialize Translator
        const translator: AutoYTranslator = new AutoYTranslator(config);
        ...
        for (const locale of config.locales) {
            if (locale === config.defaultLocale) {
                ...
                try {
                    ...
                    const result = await this.generateAutoYTranslationsSetAsync(translator, locale, availableKeys, defaultI18n, translations);
                    if (!result) {
                        return result;
                    }
